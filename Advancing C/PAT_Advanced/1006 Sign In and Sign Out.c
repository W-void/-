...
Note: It is guaranteed that the records are consistent. That is, the sign in time must be earlier than the sign out time for
each person, and there are no two persons sign in or out at the same moment.
Sample Input:
3
...
int main()
{
  int M;
  scanf("%d", &M);
  char ss[] = "23:59:59", ee[] = "00:00:00";
  char In[20], Out[20];
  while(M--){
    char id[20], s[10], e[10];
    scanf("%s%s%s", id, s, e);
    if(strcmp(s, ss)<0){
      strcpy(In, id);
      strcpy(ss, s);
